...
const DropZone = ({ elementId }: DropZoneProps): JSX.Element => {
  ...
  useEffect(() => {
    const dropZone = dropZoneRef.current;
    if (dropZone) {
      const isDragged = drag.elements.find((e) => e.getId() === elementId);
      let isChildOfDragged = false;

      drag.elements.forEach((element) =>
        element.forEachRecursive((child) => {
          if (child.getId() === elementId) isChildOfDragged = true;
          if (isRecursiveElement(child) && child.getChildrenId() === elementId) isChildOfDragged = true;
        })
      );
